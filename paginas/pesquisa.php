<div class="well well-sm">
    <div class="banner barra-topo">
        <h1 style="margin: 0% 5%; margin-down: 5%;"><br>Resultados</h1>
    </div>
    <?php
        include_once 'conexao.php';
        if (isset($_POST['buscar']) && $_POST['buscar'] === 'find') {
            $busca = mysqli_real_escape_string($mysqli, $_POST['busca']);
        
            $sql = "SELECT * FROM posts WHERE titulo LIKE '%$busca%' OR topico LIKE '%$busca%' ORDER BY id DESC";
            $seleciona = mysqli_query($mysqli, $sql);
            $conta = mysqli_num_rows($seleciona);
        
            if ($conta <= 0) {
                echo "<code>Nenhuma postagem encontrada com a palavra-chave '$busca'!</code>";
            }else{
